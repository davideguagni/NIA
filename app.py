...
from flask import Flask, request, jsonify
from metahuman import create_metahuman
from animations import apply_animation
from voice_control import recognize_speech
from vr_export import export_to_vr

app = Flask(__name__)

@app.route("/genera_metahuman", methods=["POST"])
def genera_metahuman():
    data = request.get_json()
    descrizione = data.get("descrizione", "")
    create_metahuman(descrizione)
    return jsonify({"messaggio": "MetaHuman generato con successo!"})

@app.route("/applica_animazione", methods=["POST"])
def applica_animazione():
    data = request.get_json()
    metahuman = data.get("metahuman", "MH_Example")
    animazione = data.get("animazione", "Idle")
    apply_animation(metahuman, animazione)
    return jsonify({"messaggio": f"Animazione {animazione} applicata a {metahuman}."})

@app.route("/controllo_vocale", methods=["GET"])
def controllo_vocale():
    recognize_speech()
    return jsonify({"messaggio": "Comando vocale elaborato."})

@app.route("/esporta_vr", methods=["POST"])
def esporta_vr():
    data = request.get_json()
    metahuman = data.get("metahuman", "MH_Example")
    export_to_vr(metahuman)
    return jsonify({"messaggio": f"{metahuman} esportato in VR!"})
...
print("🔹 Avvio NIA con controllo aggiornamenti automatico...")
auto_update()

from software_manager import check_and_install_software

print("🔹 Verifica software essenziali per il funzionamento di NIA...")
